...
$("save-confusion").addEventListener("click",()=>{const q=session.queue[session.index],note=$("confusion-input").value.trim();if(note.length<15)return toast("Write a short explanation of what fooled you.");state.confusions.push({date:Date.now(),concept:q.concept,note});state.repairs++;ensureDaily();state.daily.repairs++;const cs=conceptState(q.concept);if(cs.highConfidenceMisses>0){cs.highConfidenceMisses--;state.highConfidenceMisses=Math.max(0,state.highConfidenceMisses-1)}session.shield=Math.min(100,session.shield+12);session.xp+=25;addXP(25,"misconception repaired");$("shield-pill").textContent=`Shield ${session.shield}%`;$("save-confusion").disabled=true;$("save-confusion").textContent="Repaired ✓";checkDailyBonus();checkAchievements();saveState()});
...
$("show-feynman-key").addEventListener("click",()=>{const q=session.feynman,txt=$("feynman-answer").value.trim();$("feynman-key").innerHTML=`<strong>Key idea to compare against</strong><p>${esc(q.explanation)}</p><p class="muted">Transfer: ${esc(q.transfer)}</p>${txt.length<50?`<p class="muted">Your explanation is too short to earn the teach-back reward. Add the why and a use case, then compare again.</p>`:"<p class=\"muted\">Teach-back requirement met. Compare your explanation with the key idea before claiming XP.</p>"}`;$("feynman-key").classList.remove("hidden");$("finish-feynman").classList.toggle("hidden",txt.length<50)});
